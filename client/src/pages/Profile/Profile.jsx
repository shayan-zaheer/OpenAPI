  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const { data } = await axios.get(`http://localhost:8000/user/${profileId}`);
        setUser(data?.user);
      } catch (error) {
    ...
    <div className="w-full min-h-screen bg-[#1a1c1ff8] flex flex-col items-center justify-start overflow-hidden">
      <div className="w-9/12 relative mx-auto md:rounded-2xl mt-24">
        <div className="absolute inset-0 w-full h-full bg-cover bg-center blur-md rounded-2xl">
          <img src={"/HeroBackground.png"} alt="Profile Background" className="object-cover rounded-2xl w-full h-full" />
        </div>

        <div className="relative w-full min-h-full bg-black bg-opacity-60 md:rounded-2xl p-10 shadow-lg flex flex-col items-center">
          <img src={user?.profilePhoto || "/default-avatar.png"} alt="Profile" className="w-24 h-24 rounded-full border-2 border-[#6A00F4] shadow-md" />

          <h2 className="text-white text-3xl font-semibold mt-4">{user?.username}</h2>
      ...
      <div className="w-9/12 p-8 mt-10">
        <motion.h1 initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-white text-4xl font-semibold text-center">
          {loggedInUser?._id === user?._id ? "Your Uploaded " : `${user.username}'s Uploaded `}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {apis.length > 0 ? (
            ...
            <p className="text-gray-400 mt-4 text-center">{loggedInUser?._id === user?._id ? "You haven't uploaded any APIs yet." : "This user has not uploaded any APIs yet."}</p>
          )}
        </div>
      </div>
    </div>
